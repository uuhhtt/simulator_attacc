...
def main():
    ...
    parser.add_argument(
        ...
        help="dgx (each GPU has 80GB HBM), \
              ...
              tasa (single Tasa 3D-stacked chip)")
    parser.add_argument(
        "--gpu",
        type=str,
        default='A100a',
    ...
    ## set tasa configuration
    parser.add_argument("--ncores",
                        type=int,
                        default=48,
                        help="Tasa: total cores on logic die (48/60/72)")
    parser.add_argument("--core_ratio",
                        type=str,
                        default='3:1',
                        help="Tasa: P-core to E-core ratio (3:1 or 5:1)")
    parser.add_argument("--ndevice",
                        type=int,
                        default=1,
                        help="Tasa: number of Tasa chips")
    parser.add_argument("--bw_sharing",
                        action='store_true',
                        help="Tasa: enable bandwidth sharing from P-cores to E-cores")
    ...
    elif args.system == 'tasa':
        print("Tasa: {} cores (P:E={}), {} device(s), bw_sharing={}, "
              "[Lin, Lout, batch]: {}".format(
                  args.ncores, args.core_ratio, args.ndevice,
                  args.bw_sharing,
                  [args.lin, args.lout, args.batch]))
    else:
    ...
    if args.system == 'tasa':
        # Tasa system: single 3D-stacked chip
        tasa_config = make_tasa_config(
            num_cores=args.ncores,
            core_ratio=args.core_ratio,
            num_devices=args.ndevice,
            bw_sharing=args.bw_sharing)
        # Create system with a dummy GPU config (not used for Tasa)
        xpu_config = make_xpu_config(gpu_device, num_gpu=1)
        system = System(xpu_config['GPU'], modelinfos)
        system.set_tasa(modelinfos, tasa_config)

        print("---Run Tasa mode Batch {} Lin {} Lout {} pipe {} bw_sharing {}---".
              format(args.batch, args.lin, args.lout, args.pipeopt,
                     args.bw_sharing))
        perfs = []
        system.simulate_tasa(args.batch, args.lin, args.lout,
                             perfs=perfs,
                             pipe=args.pipeopt,
                             bw_sharing=args.bw_sharing)
        ...
        xpu_config = make_xpu_config(gpu_device, num_gpu=num_gpu,
                                     mem_cap=gmem_cap)
        system = System(xpu_config['GPU'], modelinfos)
        if args.system in ['dgx-attacc']:
            ...
            system.set_accelerator(modelinfos, DeviceType.CPU,
                                   xpu_config['CPU'])
